...
@section('content')
    ...
    <form action="{{ url('news/update',$news->id) }}" method="POST" enctype="multipart/form-data" class="container bg-white shadow py-3 mt-3 rounded">
        @csrf
        <div class="row">
            <div class="col-4 d-flex flex-column">
                <img src="https://res.cloudinary.com/daqpxbuu3/image/upload/v1674389086/{{ $news->photo }}" class="img-thumbnail">
                <label for="news-photo" class="btn btn-purple align-self-center mt-3">Choose a photo</label>
            ...
            <div class="col-8">
                <div class="my-2">
                    ...
                    <input type="text" name="title" id="title" value="{{ $news->title }}" class="form-control" placeholder="News title">
                    @error('title')
                ...
                <div class="my-3">
                    ...
                    <select name="category" id="category" class="form-select" disabled>
                        ...
                        <option value="important" @if($news->category == 'important') selected @endif>Important News</option>
                        <option value="normal" @if($news->category == 'normal') selected @endif>Normal News</option>
                    </select>
                    @error('category')
                        <p class="text-danger">{{ $message }}</p>
                    @enderror
        ...
        <div class="row mt-3">
            <div class="col">
                <button class="btn btn-purple float-end">Update</button>
            </div>
        </div>
    </form>
@endsection
